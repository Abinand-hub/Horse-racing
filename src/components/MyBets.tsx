    ...
    <div className="space-y-5 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-emerald-900/40">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-400 mt-0.5">
            Grouped by live contested race fixtures with your active runners & odds
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-5">
          {groupedContests.map((contestGroup) => (
            <div
              key={contestGroup.race_id || contestGroup.race_name}
              id={`contest-card-${contestGroup.race_id}`}
              className="relative rounded-3xl bg-[#091510] border-2 border-[#e5b869]/80 shadow-[0_0_22px_rgba(229,184,105,0.32)] ring-1 ring-[#e5b869]/50 overflow-hidden transition-all duration-300 hover:shadow-[0_0_30px_rgba(229,184,105,0.45)] space-y-0 group"
            >
              ...
              {/* ---------------- 1. CONTEST HEADER ---------------- */}
              <div className="p-4 sm:p-5 bg-[#050e0a]/90 border-b border-emerald-900/60 flex flex-col md:flex-row md:items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-2xl bg-[#18160c] border border-[#e5b869]/60 text-[#e5b869] flex items-center justify-center shadow-[0_0_12px_rgba(229,184,105,0.3)] shrink-0">
                    <Zap className="w-5 h-5 fill-current animate-pulse text-[#e5b869]" />
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-3 self-start md:self-auto flex-wrap">
                  ...
                  {onSelectRace && contestGroup.race_id && (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#18160c] hover:bg-[#252213] text-[#e5b869] hover:text-white border border-[#e5b869]/50 text-xs font-bold transition shadow cursor-pointer active:scale-95"
                    >
              ...
              {/* ---------------- 2. ALL SELECTIONS INSIDE THIS CONTEST ---------------- */}
              <div className="p-3 sm:p-5 space-y-2.5 bg-[#07100b]/60">
                <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400 px-1 pb-1 flex items-center justify-between">
                  <span>Selections & Odds Done By You in this Match</span>
                  <span className="text-[#e5b869] font-mono font-normal">Active Contests</span>
                </div>

                <div className="grid grid-cols-1 gap-2.5">
                  {contestGroup.bets.map((bet, idx) => (
                    <div
                      key={bet.id}
                      id={`bet-row-${bet.id}`}
                      className="bg-[#050d09] hover:bg-[#0c1c14] rounded-2xl border border-emerald-900/60 p-3 sm:p-4 transition-all duration-200 shadow-md space-y-3 relative overflow-hidden"
                    >
