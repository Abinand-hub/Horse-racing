...
export const WithdrawModal: React.FC<WithdrawModalProps> = ({
  ...
  const [amount, setAmount] = useState<number>(Math.min(2000, Math.max(500, withdrawableAmount)));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (amount > withdrawableAmount) {
      setError(`Cannot withdraw more than withdrawable balance (₹${withdrawableAmount.toLocaleString('en-IN')})`);
      return;
    }
    ...
    try {
      ...
      setSuccessMsg(`Withdrawal request of ₹${amount.toLocaleString('en-IN')} submitted successfully!`);
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="bg-slate-950 rounded-xl p-3.5 border border-slate-800 space-y-2 text-xs">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold text-rose-400">-₹{userExposure.toLocaleString('en-IN')}</span>
                </div>
                <div className="h-px bg-slate-800" />
                <div className="flex items-center justify-between">
                  <span className="font-semibold text-emerald-400">Available to Withdraw:</span>
                  <span className="font-bold text-emerald-400 text-sm">₹{withdrawableAmount.toLocaleString('en-IN')}</span>
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-[11px] text-slate-500">Min: ₹500 | Max: ₹{withdrawableAmount.toLocaleString('en-IN')}</p>
              </div>
